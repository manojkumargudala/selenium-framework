...
public class ExtentReporterNG implements IReporter {
  ...
  public void generateReport(final List<XmlSuite> xmlSuites, final List<ISuite> suites,
    ...
    extent = new ExtentReports(
        outputDirectory + File.separator + GenericConstants.EXTENT_REPORT_FILE_NAME, true,
        NetworkMode.ONLINE);
    File extenrReportConfig = new File(FilePathUtils
        .getFullPathOfFileFromClassPath(GenericConstants.EXTENT_REPORTS_XML_FILE_NAME));
    ...
    for (ISuite suite : suites) {
      ...
      for (ISuiteResult suiteResult : result.values()) {
        ...
        extendSuite.log(context.getPassedTests().size() > context.getFailedTests().size()
            ? LogStatus.PASS : LogStatus.FAIL, "setting the status");
  ...
  private void buildTestNodes(final Collection<ITestNGMethod> excludedMethods,
      final LogStatus status, final ExtentTest extendSuite) {
  ...
  private void generateExtentTest(final LogStatus status, final ExtentTest extendSuite,
    ...
    String screenShotFileName =
        (String) result.getAttribute(GenericConstants.SCREEN_SHOT_REPORTER_ATTRIBUTE);
    logger.debug("the screen shot file name is " + screenShotFileName);
    String screenCastfileName =
        (String) result.getAttribute(GenericConstants.SCREEN_CAST_REPORTER_ATTRIBUTE);
